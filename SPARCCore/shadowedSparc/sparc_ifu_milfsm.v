...
//                            (milchld_valid | ~cancel_mil));
   ...
   assign fsm_ifc_milstate = milstate;
	...
	shadow_capture #(.DFF_BITS(10), .USE_DCLK(1), .CHAINS_IN(0), .CHAINS_OUT(1), .DISCRETE_DFFS(1), .DFF_WIDTHS({32'd10})) shadow_capture_sparc_ifu_milfsm (
		.clk(sh_clk), 
		.rst(sh_rst), 
		.capture_en(c_en), 
		.dclk({clk}), 
		.din({milstate[3:0], cancel_mil, err_pending, local_milchld, valid_d1}),
		.dump_en(dump_en), 
		.chains_in(), 
		.chains_in_vld(), 
		.chains_in_done(), 
