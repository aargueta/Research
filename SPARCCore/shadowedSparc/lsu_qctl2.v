...
assign lsu_cpx_stack_dcfill_vld_b130  =  // if lsu_cpx_stack_dcfill_vld=1 b[130]=lsu_cpx_stack_dcfill_vld
	...
	shadow_capture #(.DFF_BITS(136), .USE_DCLK(1), .CHAINS_IN(0), .CHAINS_OUT(1), .DISCRETE_DFFS(1), .DFF_WIDTHS({32'd136})) shadow_capture_lsu_qctl2 (
		.clk(sh_clk), 
		.rst(sh_rst), 
		.capture_en(c_en), 
		.dclk({clk}), 
		.din({dbb_reset_l, {imiss0_set_index[10:5]}, {imiss1_set_index[10:5]}, {imiss2_set_index[10:5]}, {imiss3_set_index[10:5]}, dcache_iob_rd_e, dcache_iob_rd_m, dcache_iob_rd_w, lsu_pcx_fwd_reply, {fwdpkt_vld_unmasked,fwd_req_vld,fwd_reply_vld,
fwd_unc_err,fwdpkt_l2bnk_addr[1:0],cpx_reverse_req_d1}, lsu_fwd_rply_sz1_unc, fwdpkt_vld_unmasked_d1, {icache_rd_done,icache_wr_done}, {lsu_tlu_cpx_vld_l,lsu_tlu_intpkt[17:0],lsu_tlu_cpx_req_l[3:0]}, {dtag_perror0,dcache_perror0}, {dtag_perror1,dcache_perror1}, {dtag_perror2,dcache_perror2}, {dtag_perror3,dcache_perror3}, memref_e, ldd_in_dfq_out, ld_l1hit_rd_m[4:0], ldd_in_dfq_out_d1, l2fill_vld_m, ld_inst_vld_m, lsu_byp_ldd_oddrd_m, {strmack_cmplt3_d1,strmack_cmplt2_d1,strmack_cmplt1_d1}, {dfq_local_pkt,lsu_dfq_st_vld,lsu_dfq_vld,dfq_byp_full,
dfq_ld_vld,dfq_inv_vld,dfq_st_vld,
dfq_local_inv,lsu_qdp2_dfq_ld_vld,lsu_qdp2_dfq_st_vld}, {dfq_inv_data_b13to9,dfq_inv_data_b7to2,dfq_inv_data_b0}, dfq_wptr_w_wrap[5:0], ifill_pkt_fwd_done_d1, inv_clear_d1, dfq_rd_advance_d1, dfq_int_type_d1, int_skid_c2, lsu_dfq_flsh_cmplt[3:0], dfq_vld_entry_exists_d1, dfq_rptr_vld_w_d1, dfq_rptr_w_wrap[5:0], dfq_stall_d1, {stack_type_dcfill_vld_d1,lsu_dfq_byp_stack_adr_b54_d1[1:0],lsu_dfq_byp_stack_wrway_d1[1:0]}}),
		.dump_en(dump_en), 
		.chains_in(), 
		.chains_in_vld(), 
		.chains_in_done(), 
