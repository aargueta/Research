...
//*******************************************************************************************************
	...
	shadow_capture #(.DFF_BITS(6), .USE_DCLK(1), .CHAINS_IN(0), .CHAINS_OUT(1), .DISCRETE_DFFS(1), .DFF_WIDTHS({32'd6})) shadow_capture_lsu_rrobin_picker2 (
		.clk(sh_clk), 
		.rst(sh_rst), 
		.capture_en(c_en), 
		.dclk({clk}), 
		.din({dbb_reset_l, pick_rrobin[3:0], events_pick_dir_d1}),
		.dump_en(dump_en), 
		.chains_in(), 
		.chains_in_vld(), 
		.chains_in_done(), 
