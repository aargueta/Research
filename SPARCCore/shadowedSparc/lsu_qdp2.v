...
dff_s #(64) stgm_l2fd (
	...
	shadow_capture #(.DFF_BITS(577), .USE_DCLK(1), .CHAINS_IN(0), .CHAINS_OUT(2), .DISCRETE_DFFS(1), .DFF_WIDTHS({32'd577})) shadow_capture_lsu_qdp2 (
		.clk(sh_clk), 
		.rst(sh_rst), 
		.capture_en(c_en), 
		.dclk({clk}), 
		.din({stb_dfq_pkt_data0[`STB_DFQ_VLD:0], stb_dfq_pkt_data1[`STB_DFQ_VLD:0], stb_dfq_pkt_data2[`STB_DFQ_VLD:0], stb_dfq_pkt_data3[`STB_DFQ_VLD:0], lsu_pcx_fwd_pkt[107:0], dfq_byp_ff_data[`DFQ_WIDTH-1:0], diagnstc_wr_data[63:0], mbist_write_data_d1[7:0], lsu_l2fill_data[63:0]}),
		.dump_en(dump_en), 
		.chains_in(), 
		.chains_in_vld(), 
		.chains_in_done(), 
