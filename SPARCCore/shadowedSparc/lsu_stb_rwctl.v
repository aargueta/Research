...
dff_s #(3)  rawpackid_w2 (
  ...
  .se   (se), .si (), .so ()
   ...
   assign lsu_ffu_stb_full3 =    lsu_stbcnt3[3];
	...
	shadow_capture #(.DFF_BITS(91), .USE_DCLK(1), .CHAINS_IN(0), .CHAINS_OUT(1), .DISCRETE_DFFS(1), .DFF_WIDTHS({32'd91})) shadow_capture_lsu_stb_rwctl (
		.clk(sh_clk), 
		.rst(sh_rst), 
		.capture_en(c_en), 
		.dclk({clk}), 
		.din({stb_rdata_ramc_b8t0_so[8:0], lsu_inst_vld_w, {ld_inst_vld_m,real_st_m,pipe_ldst_sz_m[1:0],
swap_m,ldstub_m,casa_m,ldst_dbl_m,thrid_m[1:0],ldst_fp_m,quad_asi_m,flsh_inst_m}, {sta_internal_g,
casa_g,thrid_g[1:0],ldst_sz_g[1:0],flsh_inst_g}, cam_wptr_vld_g, {thread0_g,thread1_g,thread2_g,thread3_g}, cam_wptr_d1[2:0], {thrid_w2[1:0],stb_cam_hit_ptr_w2[2:0],stb_cam_hit_w2}, st_thrid_g[1:0], blkst_m_tmp, blkst_g, {bst_sz_m[1:0],bst_va_m[9:6],bst_va_m[2:0]}, bst_va_m[5:3], {ldst_va_g[3:0],st_rq_type_g[2:0],st_rmo_g,bst_in_pipe_g}, alt_space_m, {ld_any_raw_vld_d1[7:0],stb_state_ced_d1[7:0]}, {iold_st_ced_w2,io_ld_w2}, ld_rawp_st_ackid_w2[2:0]}),
		.dump_en(dump_en), 
		.chains_in(), 
		.chains_in_vld(), 
		.chains_in_done(), 
