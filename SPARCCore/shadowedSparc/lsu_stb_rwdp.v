...
//stb rdata buffer
   ...
   assign stb_rdata_ramd_b74_buf = stb_rdata_ramd_b74;
	...
	shadow_capture #(.DFF_BITS(136), .USE_DCLK(1), .CHAINS_IN(0), .CHAINS_OUT(1), .DISCRETE_DFFS(1), .DFF_WIDTHS({32'd136})) shadow_capture_lsu_stb_rwdp (
		.clk(sh_clk), 
		.rst(sh_rst), 
		.capture_en(c_en), 
		.dclk({clk}), 
		.din({stb_st_data_early_m[63:0], stb_st_data_g[63:0], {st_sz_bhww_g,st_sz_dw_g,st_sz_bhw_g,st_sz_wdw_g,
st_sz_b_g,st_sz_w_g,st_sz_hw_g,st_sz_hww_g}}),
		.dump_en(dump_en), 
		.chains_in(), 
		.chains_in_vld(), 
