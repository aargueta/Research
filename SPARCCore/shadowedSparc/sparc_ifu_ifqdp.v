...
//   assign  stxa_data_pkt[`CPX_THRFIELD] = lsu_ifu_asi_thrid[1:0];
   ...
   sink s2(.in (fill_addr_i2[4]));
	...
	shadow_capture #(.DFF_BITS(288), .USE_DCLK(1), .CHAINS_IN(0), .CHAINS_OUT(1), .DISCRETE_DFFS(5), .DFF_WIDTHS({32'd8, 32'd164, 32'd80, 32'd2, 32'd34})) shadow_capture_sparc_ifu_ifqdp (
		.clk(sh_clk), 
		.rst(sh_rst), 
		.capture_en(c_en), 
		.dclk({clk, rclk, clk, rclk, clk}), 
		.din({lcl_paddr_s[9:2], mil_entry0, mil_entry1, mil_entry2, mil_entry3, pcxreq_e, wraddr_f[42:4], inq_cpxpkt_i1, ifqop_i2, ifq_erb_asidata_i2[47:32], asi_va_i2[17:0]}),
		.dump_en(dump_en), 
		.chains_in(), 
		.chains_in_vld(), 
		.chains_in_done(), 
