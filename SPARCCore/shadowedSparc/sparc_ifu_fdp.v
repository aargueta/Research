...
`endif
   ...
   // 2. Icache parity check (increase fet data and top data to 34 bits)
	...
	shadow_capture #(.DFF_BITS(719), .USE_DCLK(1), .CHAINS_IN(0), .CHAINS_OUT(2), .DISCRETE_DFFS(2), .DFF_WIDTHS({32'd686, 32'd33})) shadow_capture_sparc_ifu_fdp (
		.clk(sh_clk), 
		.rst(sh_rst), 
		.capture_en(c_en), 
		.dclk({clk, rclk}), 
		.din({t0pc_f, pc_f, t0pc_s, pc_d, npc_d, pc_e, npc_e, pc_m, npc_m, pc_w, npc_w, t0_trapnpc_f, t0inst_s2, inst_d, inst_e, t0nir}),
		.dump_en(dump_en), 
		.chains_in(), 
		.chains_in_vld(), 
		.chains_in_done(), 
