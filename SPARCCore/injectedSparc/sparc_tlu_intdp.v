...
//				   indr_pcxpkt[12:11],  -- cannot send rst
   ...
   // 1. disable sending of reset/nuke/resum packets from indr -- DONE 1/6
	...
	shadow_capture #(.DFF_BITS(151), .USE_DCLK(1), .CHAINS_IN(0), .CHAINS_OUT(1), .DISCRETE_DFFS(1), .DFF_WIDTHS({32'd151})) shadow_capture_sparc_tlu_intdp (
		.clk(sh_clk), 
		.rst(sh_rst), 
		.capture_en(c_en), 
		.dclk({clk}), 
		.din({t0_rstid_i2[5:0], t0_inrr_i2[63:0], t0_invr_i3[5:0], tlu_lsu_int_ldxa_data_w2[63:0], t0_indr[10:0]}),
		.dump_en(dump_en), 
		.chains_in(), 
		.chains_in_vld(), 
		.chains_in_done(), 
