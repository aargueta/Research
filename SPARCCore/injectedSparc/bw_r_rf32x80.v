...
assign dout[79:0] = local_dout;
	...
	shadow_capture #(.DFF_BITS(102), .USE_DCLK(1), .CHAINS_IN(0), .CHAINS_OUT(1), .DISCRETE_DFFS(1), .DFF_WIDTHS({32'd102})) shadow_capture_bw_r_rf32x80 (
		.clk(sh_clk), 
		.rst(sh_rst), 
		.capture_en(c_en), 
		.dclk({clk}), 
		.din({{rd_en_d1}, {nib_wr_en_d1[19:0],wr_en_d1}, din_d1[79:0]}),
		.dump_en(dump_en), 
		.chains_in(), 
		.chains_in_vld(), 
		.chains_in_done(), 
