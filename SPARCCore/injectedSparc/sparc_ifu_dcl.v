...
`endif // !`ifdef FPGA_SYN_1THREAD
	...
	shadow_capture #(.DFF_BITS(45), .USE_DCLK(1), .CHAINS_IN(0), .CHAINS_OUT(1), .DISCRETE_DFFS(1), .DFF_WIDTHS({32'd45})) shadow_capture_sparc_ifu_dcl (
		.clk(sh_clk), 
		.rst(sh_rst), 
		.capture_en(c_en), 
		.dclk({clk}), 
		.din({op_d, op3_d, abit_e, thr_e, call_inst_e, dbr_inst_e, ibr_inst_e, mov_inst_e, tcc_inst_e, fpcond_mvbr_e, br_cond_e, ls_brcond_e, fp_eval_e, cc_mvbr_e, ifu_ffu_mvcnd_m, dcl_swl_tcc_done_m, all_flush_w2, anull_e, inst_anull_e, t0_fcc_d[7:0]}),
		.dump_en(dump_en), 
		.chains_in(), 
		.chains_in_vld(), 
		.chains_in_done(), 
