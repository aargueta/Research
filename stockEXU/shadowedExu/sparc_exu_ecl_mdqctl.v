...
shadow_capture #(.DFF_BITS(36),.USE_DCLK(1),.CHAINS_IN(0),.CHAINS_OUT(2)) shadow_capture_sparc_exu_ecl_mdqctl (
      .clk(sh_clk), 
		.rst(sh_rst), 
		.capture_en(c_en), 
      .dclk({{12{clk}}, clk, clk, clk, clk, clk, clk, clk, {10{clk}}, clk, clk, clk, clk, clk, clk, clk}), 
		.din({div_data[11:0], div_zero_unqual_m, isdiv_e, isdiv_m, isdiv_w, ismul_e, ismul_m, ismul_w, mul_data[9:0], mul_ready, mul_done_c0, mul_done_c1, mul_done_c2, mul_done_c3, ecl_div_mul_wen, mul_done}),
		.dump_en(dump_en), 
		.chains_in(), 
		.chains_in_vld(), 
		.chains_in_done(), 
