		  ...
		  taddr_t          t1_addr)
{
	...
	uart_word = 0;
	if (! XUartLite_IsReceiveEmpty(MBFW_XUL_BASE_ADDR)) {
...
void simp_uart_store(int data_byte){
	int ch = data_byte & 0xFF;
	XUartLite_SendByte(MBFW_XUL_BASE_ADDR, ch);
	return;
}

int simp_uart_read(uint32_t* data){
	if(XUartLite_IsReceiveEmpty(MBFW_XUL_BASE_ADDR)){
		return 0;
	}
	*data = *(uint32_t *) MBFW_XUL_RX_FIFO_ADDR;
	return 1;
}
