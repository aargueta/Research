...
void mbfw_snet_simp_store(uint32_t msg_type, uint64_t mac_addr, uint32_t msg_data_size, uint32_t* msg_data, taddr_t t1_addr){
    int  i;
    uint32_t *pkt = (uint32_t *) &snetp->tx_buf[snetp->received_data_count / sizeof(uint64_t)];

	snetp->recv_msg_data_size  = msg_data_size;
	snetp->received_data_count = 0;
	for(i = 0; i < msg_data_size; i++){
		((uint32_t*)snetp->tx_buf)[i] = msg_data[i];
	}
	snetp->mac_addr[0] = mac_addr >> 40 & 0xff;
	snetp->mac_addr[1] = mac_addr >> 32 & 0xff;
	snetp->mac_addr[2] = mac_addr >> 24 & 0xff;
	snetp->mac_addr[3] = mac_addr >> 16 & 0xff;
	snetp->mac_addr[4] = mac_addr >>  8 & 0xff;
	snetp->mac_addr[5] = mac_addr >>  0 & 0xff;
	
	snetp->polling_mode = 0; // DEFAULT
	//snetp->tx_tcp_checksum = pcx_pkt->data1;
