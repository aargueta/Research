...
#define HOST_MAC_ADDR 0x00004061861DD57C
...
t1_system_emulation_loop(void)
{
	...
	uint32_t err_en_cmd;
	if(simp_uart_read(&err_en_cmd)){
		if((err_en_cmd & 0x000000FF) == 'E'){
			struct shdw_cmd_pkt cmd_pkt;
			uint32_t err_ctrl = 0x0000FFFF;
			cmd_pkt.cmd = CMD_VALID;
			cmd_pkt.cmd |= CMD_ERR_EN;
			cmd_pkt.cmd |= (err_ctrl << CMD_ERR_CTRL_SHFT) & CMD_ERR_CTRL_MASK;
			send_shdw_cmd_pkt(&cmd_pkt);
		}
	}
	...
	if((pcx_pkt.addr_lo & 0x0FFFFFFFF) == 0x0FDFFFFF){
		if(pcx_pkt.data1 == 0xFFFFFFFF && pcx_pkt.data0 == 0x00000000){
			sh_fstop = 1;
			run_shdw_fsl(&shdw_state, sh_fstop, &sh_data_valid, &sh_data);
		}
		simp_uart_store('E');
		simp_uart_store('R');
		simp_uart_store('R');
	...
	default:
		...
		/* SHADOW FSL INTERFACE LOGIC */
		run_shdw_fsl(&shdw_state, sh_fstop, &sh_data_valid, &sh_data);
		switch(shdw_state){
			case IDLE:
				break;
			case CAPTURING:
				mbfw_snet_simp_store(SNET_CMD_TX, MAC_ADDR, 
		}
    }
